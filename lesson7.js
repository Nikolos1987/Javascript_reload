...
var scoreCount=document.querySelector('.score')
 ...
 scoreCount.innerHTML="<p>ваш счет равен :"+score+"</p>"
function scoreCounts(){
   scoreCount.innerHTML="<p>ваш счет равен :"+score+"</p>" //д/з добавил функцию выводящюю количество очков в реальном времени
}
...
function startGame() {
    ...
    // setTimeout(barrier, 6000);
    barrier_timer = setInterval(barrier, 6000)//появление барьера
...
function barrier() {
    ...
    while (!foodCreated) { //пока еду не создали
        ...
        if (!food_cell_classes.includes('snake-unit')) {
            ...
            food_cell.setAttribute('class', classes + 'barrier-unit');
            foodCreated = true;
        }
    }
}
